...
def run_all(inputfits,json,outtxt,z_component,species,BHB):
    ...
    ############################## Need to toggle this for automated vs manual runs!
    if type(json) == str:
        ## Use vlim from .json file
        json = interp_json.interp_json(json,fit_cmp) # This returns {'zfit':zfit, 'vlim':vlim, 'Nfit':Nfit, 'bfit':bfit}
    ...
    if(json['vlim']==0.0):
        ...
        # MW: only reading in bD's value, this fixed dict + float error
        # bD = {'value': XX.X , 'unit' : u.km / u.s}
        theta_guess = lamred, logN, bD['value']
        # theta_guess = lamred, logN, bD ### this is the line from the old code that couldn't read pyigmguesses JSON
        ...
        #fig.savefig(BHB_name+'-'+fit_cmp+"-debug.png") # Hannah's test line; edited for manual run

        ## Set up fitter
        datfit = model_fitter.model_fitter(data, trans_name, theta_guess, BHB)
        ...
        # Make triangle plot
        fig = corner.corner(datfit.samples, labels=[r'$\Delta\lambda_{\rm red}$ [\AA])', "logN", r'$\rm b_D$ [km/s]'],truths=[lamred_mcmc[0], logN_mcmc[0], bD_mcmc[0]])
        #fig, bigax = corner.corner(datfit.samples, labels=[r'$\Delta\lambda_{\rm red}$ [\AA])', "logN", "$\rm b_D$ [km/s]"],truths=[lamred_mcmc[0], logN_mcmc[0], bD_mcmc[0]]) # Added this instead of above line for manual run
        #bigax.ticklabel_format(useOffset=False) # Added for manual run

        ind_b_plot = np.where((wave > fitlim_b[0]-1) & (wave < fitlim_b[1]+1))
        wv_blu_plot = wave[ind_b_plot]
        ind_r_plot = np.where((wave > fitlim_r[0]-1) & (wave < fitlim_r[1]+1))
        wv_red_plot = wave[ind_r_plot]
        modlineprof = model_lineprofile.model_lineprofile(theta_mcmc,transinfo,velres,wv_blu_plot,wv_red_plot)


        ax = fig.add_subplot(444)
        #ax.plot(wv_red,flx_red,drawstyle='steps-mid',color='black')
        ax.plot(wave,flux,drawstyle='steps-mid',color='black',lw=1) # Added for manual runs       
        ax.plot(modlineprof['modwv_r'],modlineprof['modflx_r'],color='cyan',lw=2)
        ax.ticklabel_format(useOffset=False)
        ax.xaxis.set_major_locator(MaxNLocator(3))
        ax.set_xlabel(r'$\lambda$ [\AA]')
        ax.set_xlim(min(fitlim_r[0]-0.2,transinfo['lamred0']-0.1),max(fitlim_r[1]+0.2,transinfo['lamred0']+0.1)) # Added for manual run
        ax.set_ylim(min(0.5,min(modlineprof['modflx_b']-0.1)),1.2) # Original min was 0.5
        #ax.set_ylim(ymax=1.2) # Added for manual run
        ax.plot([transinfo['lamred0'],transinfo['lamred0']], [0.0,2.0],color='grey')
        ...
        ax.xaxis.set_major_locator(MaxNLocator(3))
        ax.set_xlabel(r'$\lambda$ [\AA]')
        ax.set_xlim(min(fitlim_b[0]-0.2,transinfo['lamblu0']-0.1),max(fitlim_b[1]+0.2,transinfo['lamblu0']+0.1)) # Added for manual run
        ax.set_ylim(min(0.5,min(modlineprof['modflx_b']-0.1)),1.2) # Original min was 0.5
        ...
        fits.writeto(outdir+outfits, np.array(t),overwrite=True)


        # Output model line profiles to a text file; added this for manual run
        modelfile = 'modellineprofile_'+BHB_name+'_'+fit_cmp+'.txt'
        modlineprof['spec_b'].write_to_ascii('modellineprofile_b_'+BHB_name+'_'+fit_cmp+'.txt',overwrite=True)
        modlineprof['spec_r'].write_to_ascii('modellineprofile_r_'+BHB_name+'_'+fit_cmp+'.txt',overwrite=True)
        #data = np.array(np.array(modlineprof['modwv_r']),np.array(modlineprof['modflx_r']))

        #with open(modelfile,'w') as file:
        #    file.write("Wavelength"+"\t"+"Flux"+"\n")
        #    file.write(modlineprof['modwv_r'])
            #np.savetext(modelfile, data)
        #
