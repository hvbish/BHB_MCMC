...
json = interp_json.interp_json(jsonfil,fit_cmp)
...
if(json['vlim']==0.0):
    ...
    # Make triangle plot
    fig = corner.corner(datfit.samples, labels=["$lamred$", "$logN$", "$bD$"],
                        truths=[lamred_mcmc[0], logN_mcmc[0], bD_mcmc[0]])
    ...
    #ax.set_xlabel(r'\lambda_{red} (\AA)')
    ax.set_ylim(0.5,1.2)
    #ax.set_xlim(lamcen_r-0.5,lamcen_r+0.5)
    ...
    fig.savefig(BHB_name+'-'+fit_cmp+"-triangle-lowblim.png") # Hannah's test line
